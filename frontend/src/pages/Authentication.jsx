...
//       console.log(err);
//       let message = { message: err.response.data.message };
//       setError(message);
//     }
//   };
...
export default function Authentication() {
  ...
  const [password, setPassword] = React.useState("");
  const [name, setName] = React.useState("");
  const [error, setError] = React.useState("");  // Use string for error message
  const [message, setMessage] = React.useState("");
  ...
  let handleAuth = async () => {
    try {
      ...
      if (formState === 0) {
        ...
        const result = await handleLogin(username, password);  // Call login function
        if (result) {
          setError({ message: result });  // Set error if login failed
        }
      }
  
      if (formState === 1) {
        let result = await handleRegister(name, username, password);  // Call register function
        console.log("Registration result:", result);
        setMessage(result);
        setOpen(true);
          ...
          <Box sx={{ my: 8, mx: 4, display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <Box component="form" noValidate sx={{ mt: 1 }}>
              ...
              <TextField
                ...
                onChange={(e) => setPassword(e.target.value)}
              />
              <p style={{ color: "red" }}>{error ? error.message : ""}</p>  {/* Display error message here */}
